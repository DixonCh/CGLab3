...
namespace CGlab3
{
    ...
    public class PointWithSlope
    {
        public Point p;
        public float slope;
        public PointWithSlope(Point point, float slope)
        {
            p = point;
            this.slope = slope;
        }
    ...
    public class DLinkedList
    {
        ...
        static void Main(string[] args)
        {
            ...
            else
            {
                ...
                {
                    Console.WriteLine("-----------------------------------------");
                    Console.WriteLine("Enter the point to perform Point Inclusion Test:");
                    Console.WriteLine("-----------------------------------------");
                    string Input = Console.ReadLine();
                    string[] Number = Input.Split(',', ' ');
                    Point inclusionTestpoint = new Point(Convert.ToInt32(Number[0]), Convert.ToInt32(Number[1]));
        ...
        public static Point[] arrangepointsforpolygon(Point[] points)
        {
            int pointCount = points.Count();
            Point[] arrangePoints = new Point[pointCount];
            PointWithSlope[] pointWithSlope = new PointWithSlope[pointCount - 1];
            Point basePoint = points[0];
            for (int i = 1; i < pointCount; i++)
            {
                if (basePoint.y == points[i].y)
                {
                    pointWithSlope[i - 1] = new PointWithSlope(points[i], 0);
                }
                else
                {
                    float slope = (float)(basePoint.y - points[i].y) / (basePoint.x - points[i].x);
                    pointWithSlope[i - 1] = new PointWithSlope(points[i], slope);
                }

            }
            pointWithSlope = pointWithSlope.OrderByDescending(x => x.slope).ToArray();
            int countNegativeItem = pointWithSlope.Count(x => x.slope <= 0);
            int countPositiveItem = pointWithSlope.Count() - countNegativeItem;
            PointWithSlope[] positveSlopeitem = new PointWithSlope[countPositiveItem];
            PointWithSlope[] negativeSlopeitem = new PointWithSlope[countNegativeItem];
            int j = 0;// for negative
            int l = 0; //for positive count
            foreach (var item in pointWithSlope)
            {
                if (item.slope <= 0)
                {
                    negativeSlopeitem[j] = item;
                    negativeSlopeitem[j].slope = negativeSlopeitem[j].slope * -1;
                    j++;
                }
                else
                {
                    positveSlopeitem[l] = item;
                    l++;
                }
            }
            negativeSlopeitem = negativeSlopeitem.OrderByDescending(x => x.slope).ToArray();
            positveSlopeitem = positveSlopeitem.OrderBy(x => x.slope).ToArray();
            int k = 0;
            l = 0;
            PointWithSlope[] pointWithSlopeOutput = new PointWithSlope[pointCount - 1];
            for (int i = 0; i < pointCount - 1; i++)
            {
                if (i >= pointCount - countNegativeItem - 1)
                {
                    pointWithSlopeOutput[i] = negativeSlopeitem[k];
                    k++;
                }
                else
                {
                    pointWithSlopeOutput[i] = positveSlopeitem[l];
                    l++;
                }

            }
            arrangePoints[0] = points[0];
            for (int i = 0; i < pointCount - 1; i++)
            {
                arrangePoints[i + 1] = pointWithSlopeOutput[i].p;
            }
